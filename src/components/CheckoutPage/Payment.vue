<template>
  ...
  <ConfirmAddDonationModal
    :ModalConfirmAddDonation="ModalConfirmAddDonation"
  ...
  <div class="">
    <div class="p-10 bg-white dark:bg-[#28334e]">
      ...
      <div class="mb-6">
        <label class="block mb-3 text-gray-600 dark:text-slate-200" for=""
...
const minCardMonth = computed(() =>
  props.formData.cardYear === minCardYear.value ? new Date().getMonth() + 1 : 1
)
...
const maskCardNumber = () => {
  ...
  props.formData.cardNumber = props.formData.cardNumber
    .split('')
    .map((c, i) => (i > 4 && i < 14 && c.trim() ? '*' : c))
    .join('')
}
...
const finishPayment = () => {
  ...
  if (
    !props.formData.cardName ||
    !props.formData.cardNumber ||
    !props.formData.cardMonth ||
    !props.formData.cardYear ||
    !props.formData.cardCvv
  ) {
    showToast({
  ...
  if (
    ...
    dinersPattern.test(cardNumber) ||
    defaultPattern.test(cardNumber)
  ) {
  ...
  if (
    props.formData.cardYear < minCardYear.value ||
    (props.formData.cardYear == minCardYear.value && props.formData.cardMonth < minCardMonth.value)
  ) {
    showToast({
