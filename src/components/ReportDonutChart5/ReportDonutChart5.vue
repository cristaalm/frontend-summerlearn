...
watch(donations, (newValue) => {
  if (newValue) {
    ...
    if (Array.isArray(newValue) && newValue.length > 0) {
      ...
      console.log("Suma total de donaciones:", totalDonations.value);
      ...
      console.log("Suma total de gastos de donaciones:", totalSpent.value);
    }
  }
});
...
const chartData = computed(() => {
  console.log("chartData:", [totalDonations.value, totalSpent.value]);
...
<template>
  <div>
    <Chart v-if="!loading" type="doughnut" :width="props.width" :height="props.height" :data="data" :options="options" />
