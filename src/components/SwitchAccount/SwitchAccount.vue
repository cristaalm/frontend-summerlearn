...
<template>
  <Dialog :open="props.switchAccount" @close="() => {
      ...
      <Dialog.Description class="px-2.5 pt-3.5 pb-4">
        <div class="flex flex-col gap-1.5">
          <template v-for="(faker, fakerKey) in _.take(users.fakeUsers(), 5)" :key="fakerKey">
            <FormCheck.Label :for="'switch-account-' + fakerKey"
              class="flex items-center px-2.5 py-1 rounded-lg hover:bg-slate-100 cursor-pointer">
              <div class="overflow-hidden rounded-full w-11 h-11 image-fit border-[3px] border-slate-200/70">
                <img alt="Tailwise - Admin Dashboard Template" :src="faker.photo" />
              </div>
              <div class="ml-3.5">
                <div class="font-medium dark:text-slate-200">{{ faker.name }}</div>
                <div class="text-xs text-slate-500 mt-0.5">
                  {{ faker.email }}
                </div>
              </div>
              <div class="relative ml-auto w-7 h-7">
                <FormCheck.Input :id="'switch-account-' + fakerKey" type="checkbox" value="switch-account"
                  :checked="switchAccount === fakerKey" @change="() => {
