...
const { validate, valid, status, name, birthdate, curp, validateInputCurp, resetFields, validateAll, setValueTrueAll } = useValidationAddChildren()
...
const isChange = computed(() => {
    return name.value !== props.infoChild.name || birthdate.value !== formatDateToDDMMYYYY(props.infoChild.birthdate) || curp.value !== props.infoChild.curp || isChangeImage.value
})


watch(() => props.infoChild, (newVal) => {
    if (newVal) {
...
const handleUpdateChild = () => {
    ...
    updateChild({ child: props.infoChild, newChild: newChild, setModalEditChild: props.setModalEditChild })
}
    ...
    <Dialog size="xl" :open="ModalEditChild" @close="() => {
        ...
        <Dialog.Panel class="text-center flex flex-col">
            <div
                ...
                <h2 class="text-2xl font-bold text-black dark:text-slate-200">Editar información del niño</h2>
                            ...
                            <div class="flex flex-row items-center justify-center">
                                <div
                                    ...
                                    <div @click="triggerFileSelect"
                                        ...
                                        <Lucide icon="Pencil"
                                            class="w-3.5 h-3.5 stroke-[1.3] text-slate-500 dark:text-slate-200" />
                                    </div>

                                    <!-- Input tipo archivo oculto -->
                                    <input id="profileImageInput" name="profileImage" type="file" class="hidden"
                                        @change="validateImage" accept="image/*" />
                                </div>

                                <Button variant="outline-success" size="sm"
                                    :disabled="!isChangeImage || loadingEditChild"
                                    class="pl-3.5 pr-4 mr-2 h-8 ml-8 dark:border-green-400 dark:text-green-400"
                                    @click="resetImage">
            ...
            <div class="flex py-5 md:justify-end px-7">
                <Button variant="outline-success" class="w-full px-10 md:w-auto dark:text-slate-200"
                    @click="handleUpdateChild" :disabled="loadingEditChild || !valid || !isChange">
                    <Lucide v-if="!loadingEditChild" icon="Save"
                        class="stroke-[1.3] w-4 h-4 mr-2 -ml-2 text-success dark:text-slate-200" />
                    <LoadingIcon v-if="loadingEditChild" icon="tail-spin" class="stroke-[1.3] w-4 h-4 mr-2 -ml-2"
                        color="black" />
