...
const handleDeleteArea = () => {
    deleteProgram({ program: props.infoProgram, programs: programs, setModalDeleteProgram: props.setModalDeleteProgram })
}
</script>

<template>
    <Dialog :open="ModalDeleteProgram" @close="() => {
        setModalDeleteProgram({ open: false })
    }">
        <Dialog.Panel>
            <div class="p-5 text-center">
                <div class="w-16 h-16 mx-auto mt-3 ">
                    <Lucide v-if="!loadingDeleteProgram" icon="XCircle"
            ...
            <div class="px-5 pb-8 text-center space-x-8">
                ...
                <Button type="button" variant="danger" class="w-24 text-slate-900 dark:text-slate-200"
                    @click="handleDeleteArea" ref="deleteButtonRef">
                    Eliminar
                </Button>
            </div>
        </Dialog.Panel>
