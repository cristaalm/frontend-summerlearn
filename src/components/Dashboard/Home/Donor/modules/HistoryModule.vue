...
const loading = ref(true); // Estado para controlar la carga

onMounted(async () => {
  await loadDonations();
  loading.value = false; // Cambia el estado a false una vez que se cargan las donaciones
});
...
<template>
  <div class="flex flex-col col-span-12 md:col-span-6 lg:col-span-8 gap-2 p-5 box box--stacked">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
      ...
      <template v-else>
        <div class="flex justify-center w-full p-4 text-slate-500 dark:text-slate-400">No hay donaciones disponibles</div>
      </template>
    </div>
  </div>
</template>
