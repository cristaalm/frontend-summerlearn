...
<template>
  <div
    class="flex flex-col col-span-12 md:col-span-6 lg:col-span-8 gap-2 p-5 box box--stacked"
    id="ultimasDonaciones"
  >
    <div
    ...
    >
      <div class="relative flex flex-col gap-5 lg:gap-8">
        <div>
          <div class="text-md uppercase text-slate-500 dark:text-slate-400">
            Últimas 6 donaciones:
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
      <template v-if="lastSixDonations.length">
        <div
          id="formatoDonacion"
          v-for="(donation, index) in lastSixDonations"
          :key="index"
      ...
      <template v-else>
        <div class="flex justify-center w-full p-4 text-slate-500 dark:text-slate-400">
