...
if (status == 1) {
  ...
  const notification = computed(
    () => chats.value.findIndex((chat) => chat.seenChat === false) !== -1
  )
...
async function againTour() {
  ...
  if (role == 5) startTourBenef(router, id, formattedMenu, activeMobileMenu, showToast)
}
</script>
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <a
            class="flex items-center duration-300 group-[.side-menu--collapsed]:xl:ml-2 group-[.side-menu--collapsed.side-menu--on-hover]:xl:ml-0 cursor-pointer hover:scale-105 transition-all"
            @click="
              (event: MouseEvent) => {
                event.preventDefault()
                router.push({
              ...
              <li v-else :key="menuKey" :id="`sideBar-${menu.pageName}`">
                ...
                >
                  ...
                  <span v-if="notification && menu.pageName == 'chat'" class="relative">
                    <span
                      class="absolute bottom-1 -left-6 w-2 h-2 bg-theme-1 rounded-full dark:bg-blue-500"
                    ></span>
                  </span>
        ...
        >
          ...
          >
            ...
            <div class="flex justify-end px-4 space-x-5">
              <Menu>
                <Menu.Button
                  class="overflow-hidden rounded-full w-9 h-9 border-3 border-white/15 image-fit"
                >
                  <!-- Verifica si loadingUserPhoto es falso y photoUser tiene un valor válido -->
                  <img
                    alt="User Photo"
                    v-if="!loadingUserPhoto"
                    :src="`${Baseurl}${photoUser}`"
