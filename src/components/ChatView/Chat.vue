...
const copiedEmail = () => {
  navigator.clipboard
    .writeText(selectChatInfo.value.user.email)
    .then(() => {
  ...
  <template v-if="selectedChatIndex !== 'chats' && selectedChatIndex !== 'contacts'">
    <div class="flex flex-col w-full lg:w-2/3 lg:p-5 lg:pt-0 gap-y-7">
      <div class="flex flex-col p-5 box box--stacked" id="chat">
        <div
          class="flex items-center gap-3.5 border-b border-dashed border-black dark:border-slate-400 pb-5"
        >
          <div class="relative">
            <div
            ...
            >
              <img
                ...
                :src="`${Baseurl}${selectChatInfo.user.userPhoto}`"
              />
            </div>
            <div
              v-if="selectChatInfo.user.isOnline"
          ...
          <div>
            <div
              class="font-medium truncate max-w-[9rem] md:max-w-none text-black dark:text-slate-200"
            >
              {{ selectChatInfo.user.name }}
            </div>
            <div
              class="text-slate-500 mt-0.5 truncate max-w-[9rem] md:max-w-none dark:text-slate-400"
            >
          ...
          <div class="flex gap-2 ml-auto">
            ...
            >
              <Lucide
                icon="Mail"
                class="w-4 h-4 text-primary dark:text-slate-200 fill-primary/10"
              />
            </Tippy>
          </div>
        </div>
        ...
        >
          ...
          <div
            v-if="!loadingMessages && messagesSelectChat && messagesSelectChat.length === 0"
            class="flex items-center justify-center h-[530px]"
          >
            <div class="text-slate-500 dark:text-slate-400">No hay mensajes, ¡escribe algo!</div>
          </div>
          ...
          <div
            class="flex flex-col gap-3.5 py-5 px-3"
            v-if="messagesSelectChat && messagesSelectChat.length > 0"
          >
            <div
              v-for="(message, index) in messagesSelectChat"
              :key="index"
            ...
            >
              ...
              <div
                :class="[
                  `border p-2 rounded-r-xl rounded-tl-xl group-[.right]:text-right group-[.right]:rounded-l-xl group-[.right]:rounded-br-none`,
                  message.user == selectChatInfo.user.id
                    ? 'bg-slate-50 border-slate-200/80 dark:bg-slate-800/80 dark:border-slate-700/80'
                    : 'bg-slate-300 border-slate-300/80 dark:bg-slate-600 dark:border-slate-600/80'
                ]"
              >
                <div
        ...
        <div class="relative">
          ...
          <div class="absolute inset-y-0 right-0 flex items-center justify-center w-[3.8rem]">
            ...
            >
              <Lucide
                v-if="!loadingMessages && !loadingSendMessage"
                icon="Send"
