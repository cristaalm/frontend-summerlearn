...
<template>
    ...
    <div class="h-[530px] gap-1 flex flex-col" id="listaConversaciones">
        ...
        >
          ...
          <div class="relative flex-[0.1]">
            ...
            >
              <img
                ...
                :src="`${Baseurl}${chat.user.userPhoto}`"
              />
            </div>
            <div
          ...
          <div class="flex-[0.9] w-full">
            ...
            <div class="flex items-center mt-1.5">
              <div
                class="text-slate-500/90 max-w-[9rem] md:max-w-[12rem] truncate dark:text-slate-400"
              >
                {{
                  chat?.lastMessage
      ...
      <template v-else>
        <div class="flex items-center justify-center h-[530px]">
          <div class="text-slate dark:text-slate-400">No se encontraron chats</div>
        </div>
      </template>
    </div>
  </template>
  <template v-if="!loadingChats && chats.length === 0">
    <div class="flex items-center justify-center h-[530px]">
      <div class="text-slate dark:text-slate-400">No hay chats, ¡empieza uno!</div>
    </div>
  </template>
