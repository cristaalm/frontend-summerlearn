...
<template>
  <div>
    <Slideover :open="props.notificationsPanel" @close="() => {
          ...
          <div class="flex flex-col p-3 gap-0.5">
            <template v-for="(faker, fakerKey) in activities.fakeActivities()" :key="fakerKey">
              <a href="" class="flex items-center px-3 py-2.5 rounded-xl hover:bg-slate-100/80">
                <div>
                  <div class="overflow-hidden border-2 rounded-full w-11 h-11 image-fit border-slate-200/70">
                    <img alt="Tailwise - Admin Dashboard Template" :src="users.fakeUsers()[0].photo" />
                  </div>
                </div>
                <div class="sm:ml-5">
                  <div class="font-medium dark:text-slate-200">{{ faker.activity }}</div>
                ...
                <template v-if="_.random(0, 1) == 1">
                  <div class="flex-none w-2 h-2 ml-auto border rounded-full bg-primary/40 border-primary/40"></div>
                </template>
              </a>
            </template>
          </div>
