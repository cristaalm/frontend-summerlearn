...
<template>
  <div v-if="props.slide">
    ...
    <div
      class="fixed bottom-0 right-0 z-50 flex items-center justify-center mb-5 mr-5 text-white rounded-full shadow-lg cursor-pointer h-14 w-14 bg-theme-1 dark:bg-theme-2 group"
      @click="(event: MouseEvent) => {
        event.preventDefault()
        setThemeSwitcherSlideover(true)
      }">
      <Lucide class="w-5 h-5 transition-transform duration-300 group-hover:scale-125" icon="Palette" />
    </div>
