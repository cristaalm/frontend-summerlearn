...
import { getExcel } from '@/services/donations/getExcel'
...
export function useExportExcel() {
  ...
  const loadExportExcel = async () => {
    ...
    try {
      ...
      link.href = url
      link.setAttribute('download', 'donations.xlsx') // Nombre del archivo descargado
      ...
      showToast({ message: 'Archivo Excel generado exitosamente', tipo: 'success' })
    } catch (error) {
      console.error(error)
