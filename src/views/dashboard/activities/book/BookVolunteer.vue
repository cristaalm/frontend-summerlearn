    ...
    <div class="flex flex-col gap-y-6" id="customList">
      ...
      <div class="bg-white dark:bg-[#28334e] w-full min-h-screen rounded-xl p-6 dark:text-slate-200">
        <div class="w-full space-y-5">
          <!-- Lista de elementos -->
          <div v-for="(item, index) in items" :key="index" class="relative flex items-center gap-3.5 p-5 box box--stacked">
            <!-- Área de información -->
      ...
      <!-- Modal Diario -->
      <Diario 
        :ModalEditUser="showDiario" 
        :setModalEditUser="closeDiario" 
        :infoUser="{}" 
      />
    </div>
  </template>
