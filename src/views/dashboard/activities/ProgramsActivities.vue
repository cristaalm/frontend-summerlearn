    ...
    <div v-else class="w-full max-w-4xl space-y-8">
      <div v-for="child in childrenData" :key="child.id" class="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg shadow-md ">
        ...
        <template v-for="(programs, areaName) in child.areas" :key="areaName">
          <template
            v-if="!programs.filter(program => program.area === areaName).every(program => program.activities.length === 0)">
            <div class="mb-6">
      ...
      <div v-if="!childrenData.length" class="w-full h-[550px] flex flex-col justify-center items-center ">
